...
namespace Services
{
    class OrderService 
    {
        ...
        public Order getById(int id)
        {
            ...
            {
                command.CommandText = "SELECT * FROM order WHERE order.order_ID=@order_ID";
                ...
                order.isPaid = bool.Parse(dataReader["isPaid"].ToString());
                dataReader.Close();
            }
            return order;
        }
        ...
        public void update(Order obj)
        {
            ...
            using (MySqlCommand command = connection.CreateCommand())
            {
                command.CommandText = "UPDATE order SET user_ID=@user_ID,date=@date,isPaid=@isPaid WHERE order_ID=@order_ID";
                command.Parameters.AddWithValue("@user_ID", obj.user_ID);
                command.Parameters.AddWithValue("@date", obj.date);
                command.Parameters.AddWithValue("@isPaid", obj.isPaid);
